...
from .forms import AddItemToCartForm, SetItemCountForm, DiscountForm, LocationForm
        ...
        if form_data.is_valid():
            ...
            if item_var.exists():
                _, created = DeliveryCartItem.objects.get_or_create(
                    cart=cart, item=item_var.first(), count=1
                )
                ...
                messages.warning(self.request, "No valid item was retrieved by us.")
...
class CartView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form_data.is_valid():
            ...
            return HttpResponseBadRequest(content="invalid form detected.")
    ...
    def post(self, *args, **kwargs):
        ...
        if form_data.is_valid():
            ...
            match op:
                case "a":
                    if promo_obj.exists():
                        try:
                            ...
                            if (
                                promo_obj.expiration_date is None
                                or datetime.now().isoformat()
                                < promo_obj.expiration_date.isoformat()
                            ):
                                self.request.user.user_cart.discounts.add(promo_obj)
                                messages.success(
                                    self.request, "Discount was added to your cart."
                                )
                            else:
                                messages.error(
                                    self.request, "This promo code is expired."
                                )
                ...
                case "d":
                    if promo_obj.exists():
                        self.request.user.user_cart.discounts.remove(promo_obj.first())
                    return redirect("delivery:cart")
...
class PurchaseView(LoginRequiredMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if not (form.is_valid() or cart.user_address is not None):
            messages.error(
                self.request, "Please make sure you have chosen a shipping address."
            )
            return redirect("delivery:cart")
        elif form.is_valid():
        ...
        try:
            ...
            for restaurant, cart_items in grouped:
                description = (
                    f"Order for mr./mrs./miss {user.name.title()}, "
            ...
            cart_qs.update(
                date_submitted=timezone.now(), payment=payment, user_address=location
            )
...
class PurchaseStatusView(LoginRequiredMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        cart_qs = DeliveryCart.objects.filter(user=self.request.user)
        if (
            ...
            return render(self.request, self.template_name, self.context)
        messages.error(self.request, "Requested cart is invalid.")
        return redirect("delivery:cart")
...
class OrdersView(LoginRequiredMixin, View):
    ...
    def get(self, *args, **kwargs):
        self.context.update(
            {
                "carts": (
                    DeliveryCart.objects.filter(user=self.request.user).exclude(
                        date_submitted=None
                    )
                )
            }
