    ...
    async def get(self, *args, **kwargs):
        ...
        if form_data.is_valid():
            ...
            await sync_to_async(messages.warning)(
                self.request, "Invalid parameters were provided. Request aborted."
            )
        return response
...
class SearchRestaurantsView(View):
    def get(self, *args, **kwargs):
        ...
        if form_data.is_valid():
            ...
            results = {
                "hits": paginated_data[0].object_list,
                "page": {
                    "has_next_page": paginated_data[0].has_next(),
                    "has_previous_page": paginated_data[0].has_previous(),
                    "next_page_number": (
                        paginated_data[0].next_page_number()
                        if paginated_data[0].has_next()
                        else False
                    ),
                    "previous_page_number": (
                        paginated_data[0].previous_page_number()
                        if paginated_data[0].has_previous()
                        else False
                    ),
...
class RestaurantPageView(View):
    ...
    def get(self, *args, **kwargs):
        restaurant = Restaurant.objects.get(public_uuid=kwargs.get("public_uuid"))
        reviews = Review.objects.filter(item__cuisine__restaurant=restaurant)
        self.context.update(
            {
        ...
        if (
            ...
            with transaction.atomic():
                try:
                    ...
                    return render(self.request, self.template_name, self.context)
            messages.success(
                self.request, "Congratualations! You're now one of our partners."
            )
