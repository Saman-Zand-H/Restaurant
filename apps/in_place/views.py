...
def prepare_order_namedtuple(restaurant, timestamp: date = timezone.now().date()):
    ...
    item_var_qs = ItemVariation.objects.filter(item__cuisine__restaurant=restaurant)
    OrderItemFormset = formset_factory(
        OrderItemForm, extra=item_var_qs.count(), max_num=item_var_qs.count()
    )
    ...
    def formset_initials(order):
        ...
        items = ItemVariation.objects.filter(item_orders__in=order.order_items.all())
        ...
        empty_items = [
            {"count": 0, "item": i, "fee": i.price or 0, "paid_price": 0}
            for i in item_diff
        ]
    ...
    min_t = datetime.combine(timestamp, time.min, timezone.utc)
    max_t = datetime.combine(timestamp, time.max, timezone.utc)
    orders = restaurant.restaurant_orders.filter(
        Q(timestamp__gte=min_t) & Q(timestamp__lte=max_t)
    )
...
class DashboardView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        score_chart, revenue_chart, sale_chart = (
            ...
            weekly_revenue_chart_data([i // 1000 for i in restaurant.weekly_revenue]),
        ...
        # if this form has previously been submitted, and
        # and resulted in an error, we send to data to this view
        # using sessions, and then re-validate this data to show the
        # client what's gone wrong.
        order_form, dinein_form, order_item_formset = (
            ...
            OrderItemFormset(
                ...
                initial=[
                    {"count": 0, "item": i, "fee": i.price or 0} for i in item_var_qs
                ],
        ...
        deque(
            map(
                ...
                [
                    i
                    for i in [order_form, dinein_form, order_item_formset]
        ...
        try:
            with transaction.atomic():
                if order_form_data.is_valid():
                    ...
                    # we have used a choicefield along with choices for dest; 
                    # so no one can take advantage of it and the code remains secure.
                    dest = order_form_data.cleaned_data.get("dest")
                    timestamp = order_form_data.cleaned_data.get(
    ...
    def handle_formset_create(
        self, formset, item_var_qs, order_type, restaurant, dest, timestamp
    ):
        formset_data = formset(
            self.request.POST,
            initial=[{"count": 0, "item": i, "fee": i.price} for i in item_var_qs],
            form_kwargs={"item_qs": item_var_qs},
        )
        if formset_data.is_valid():
            order = Order.objects.create(
                restaurant=restaurant, order_type=order_type, timestamp=timestamp
            )

            dinein_form = DineInForm(data=self.request.POST)
            if order_type == "i" and dinein_form.is_valid():
                d_cleaned = dinein_form.cleaned_data
                self._validate_table_number(
                    order, d_cleaned.get("table_number"), dinein_form
                )
                ...
                messages.error(self.request, "Invalid data was provided. Try again.")
            ...
            filled_forms = [
                *filter(lambda x: x.cleaned_data.get("count"), formset_data.forms)
            ]
            ...
            messages.success(self.request, "Order was submitted successfully.")
            return redirect(f"in_place:{dest}")

        messages.error(self.request, "Invalid input was detected. Try again.")
    ...
    def render_or_redirect(self, dest="dashboard"):
        # since this view is handling order management from different places,
        # in this way we can handle the destination of the request.
...
class EditOrderView(LoginRequiredMixin, UserPassesTestMixin, View):
        ...
        with transaction.atomic():
            if order_data.is_valid():
                ...
                if order.order_type == "i":
                    ...
                    if dinein_form.is_valid() and dinein_form.has_changed():
                        ...
                        DineInOrder.objects.select_for_update().filter(
                            order__public_uuid=order.public_uuid
                        ).update(
                ...
                if formset_data.is_valid():
                    ...
                    changed_forms = [*filter(lambda x: x.has_changed(), formset_forms)]
                    for form in changed_forms:
                        ...
                        o, _ = OrderItem.objects.get_or_create(order=order, item=item)
                        o.count = count
                        o.paid_price = paid
                        o.save()
                    messages.success(
                        self.request, "The Order was updated successfully."
                    )
                else:
                    self._invalid_input_message()
                return redirect(f"in_place:{dest}")
    ...
    def _invalid_input_message(self):
        return messages.error(self.request, "Invalid data was provided. Try again.")
...
class DeleteOrderView(LoginRequiredMixin, UserPassesTestMixin, View):
    def test_func(self):
        return (
            hasattr(self.request.user, "user_staff") 
...
class OrdersView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        # to validate data we need to call is_valid()
        deque(
            map(methodcaller("is_valid"), [order_form, dinein_form, order_item_formset])
        )
        self.context.update(
            {
                "orders": prepare_order_namedtuple(restaurant),
        ...
        if form_data.is_valid():
            ...
            orders_data = [
                self.SearchData(
                    ...
                    timestamp=datetime.fromisoformat(i["_source"]["timestamp"]),
...
class StaffView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def test_func(self):
        return self.request.user.has_perm("in_place.mod_staff") and hasattr(
            self.request.user, "user_staff"
        )
    ...
    def get(self, *args, **kwargs):
        ...
        self.context.update(
            {
                "staff": restaurant.restaurant_staff.order_by("user__last_name"),
        ...
        if form_data.is_valid():
            with transaction.atomic():
                ...
                try:
                    ...
                    pass
        messages.error(
            self.request, "Invalid input detected. Please try again more carefully."
        )
...
class EditStaffView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def test_func(self):
        return hasattr(self.request.user, "user_staff") and self.request.user.has_perm(
            "in_place.staff"
        )
    ...
    def post(self, *args, **kwargs):
        ...
        if username_form.is_valid():
            ...
            staff_form = ChangeStaffForm(
                data=self.request.POST, initial=self._prepare_initial(username)
            )
            if staff_form.is_valid() and staff_form.has_changed():
                with transaction.atomic():
                    phonenumber = staff_form.cleaned_data.pop("phonenumber", None)
                    update_args = {
                        i: staff_form.cleaned_data.get(i)
                        for i in staff_form.changed_data
                    }
                    try:
                        if (
                            phonenumber is not None
                            and phonenumber in staff_form.changed_data
                        ):
                            get_user_model().objects.select_for_update().update(
                        ...
                        messages.success(
                            self.request, "Staff record was updated successfully."
                        )
                    except:
                        pass
            else:
...
class DeleteStaffView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if username_form.is_valid():
            ...
            messages.error(
                self.request, "Something unexpected happened. Please try again."
            )
        return redirect("in_place:staff")
...
class FinanceView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        self.context.update(
            {
                "gamma_data": json.dumps(gamma_data),
...
class SellsToExcelView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        response["Content-Disposition"] = (
            "attachment; " f"filename={restaurant.name.lower()}_orders.xls"
        )
        ...
        fields = [
            *chain.from_iterable([*qs.values_list("timestamp", "order_number", "id")])
        ]
        timestamp, order_n, order_id = (
            np.asarray([i.isoformat() for i in fields[::3]]),
            np.asarray(fields[1::3]),
            np.asarray(fields[2::3]),
        )
        data = [
            *reduce(
                lambda i, j: np.append(i, j, axis=1),
                [
        ...
        items, counts, paid_prices, order, timestamp = (
            np.asarray([ItemVariation.objects.get(id=i).full_name for i in vals[::5]]),
            np.asarray(vals[1::5]),
            np.asarray(vals[2::5]),
            np.asarray(vals[3::5]),
...
class MenuView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        cuisine_form = NewCuisineForm(data=sessions.pop("cuisine_form", None))
...
class CreateCuisineView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form_data.is_valid():
            ...
            messages.error(
                self.request, "There was an error during the process. Try again."
            )
...
class EditCuisineView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form_data.is_valid():
            public_uuid = form_data.cleaned_data.get("public_uuid")
            name = form_data.cleaned_data.get("name")
            Cuisine.objects.filter(public_uuid=public_uuid).update(name=name)
            messages.success(self.request, "The cuisine was successfully updated.")
        else:
            messages.error(
                self.request, "There was an error during the process. Try again."
            )
...
class DeleteCuisineView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form_data.is_valid():
            public_uuid = form_data.cleaned_data.get("public_uuid")
            Cuisine.objects.filter(public_uuid=public_uuid).delete()
            messages.success(self.request, "The cuisine was successfully deleted.")
        else:
            messages.error(
                self.request, "There was an error during the process. Try again."
            )
...
class CreateItemView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, *args, **kwargs):
        restaurant = self.request.user.user_staff.restaurant
        cuisine_qs = Cuisine.objects.filter(restaurant=restaurant).values(
            "public_uuid", "name"
        )
        form_data = NewItemForm(
            data=self.request.POST, files=self.request.FILES, cuisine_qs=cuisine_qs
        )
        if form_data.is_valid():
            ...
            Item.objects.create(
                cuisine=cuisine, name=name, picture=picture, description=desc
            )
            messages.success(self.request, f"{name.title()} was successfully added.")
        else:
            messages.error(
                self.request, "There was an error during the process. Please try again."
            )
...
class EditItemView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, *args, **kwargs):
        form_data = EditItemForm(data=self.request.POST, files=self.request.FILES)
        if form_data.is_valid():
            name = form_data.cleaned_data.get("name")
            desc = form_data.cleaned_data.get("description")
            public_uuid = form_data.cleaned_data.get("public_uuid")
...
class DeleteItemView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form_data.is_valid():
            ...
            item.delete()
            messages.success(self.request, f"The item was successfully deleted.")
        else:
            messages.error(
                self.request, "There was an error during the process. Please try again."
            )
...
class CreateItemVarView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, *args, **kwargs):
        restaurant = self.request.user.user_staff.restaurant
        item_qs = Item.objects.filter(cuisine__restaurant=restaurant).values(
            "public_uuid", "name"
        )
        form_data = NewItemVarForm(data=self.request.POST, item_qs=item_qs)
        if form_data.is_valid():
            ...
            ItemVariation.objects.create(
                name=name, price=price, item=item, description=description
            )
            messages.success(self.request, f"{name.title()} was successfully added.")
        else:
            messages.error(
...
class DeleteItemVarView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form_data.is_valid():
            ...
            messages.success(
                self.request, "The item-variation was successfully deleted."
            )
        else:
            messages.error(
                self.request, "There was an error during the process. Please try again."
        ...
        if form.is_valid() and form.has_changed():
            if "name" in form.changed_data:
                ...
                Restaurant.objects.filter(
                    restaurant_staff__user=self.request.user
                ).update(name=name)
...
class LocationView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        if (info := restaurant.location) is not None:
            location_initial = {
                "geo_address": info.geo_address,
                "address": info.address,
                "city": info.city.id,
                "province": info.province.id,
            }
        self.context.update({"form": LocationForm(initial=location_initial)})
        return render(self.request, self.template_name, self.context)
...
class ProvinceAjax(View):
    ...
    def get(self, *args, **kwargs):
        if (province_id := self.request.GET.get("province")) is not None and (
            province := Province.objects.filter(id=province_id)
        ).exists():
