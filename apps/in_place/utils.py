...
def get_restaurant_sells(restaurant_id: int):
...
def sells_gamma(restaurant_id: int):
    ...
    """
    sells = get_restaurant_sells(restaurant_id)
    data = {"x": [], "y": [], 
            "stdev": [], "mean": [], "med": [],
            "q1": [], "q3": []}
    if sells:
        s, loc, scale = stats.gamma.fit(sells)
        q1, med, q3 = np.quantile(sells, .25), np.median(sells), np.quantile(sells, .75)
...
def orders_geos(restaurant_id: int):
    ...
    deliv_qs = DeliveryCartItem.objects.filter(
        item__item__cuisine__restaurant__id=restaurant_id).annotate(
            deliv_cart=F("cart"))
    grouped = [*map(lambda i: i[0], groupby(deliv_qs, attrgetter("deliv_cart")))]
    points = [*filter(
        lambda i: is_not(i, None), 
        [
            DeliveryCart.objects.get(id=i).user_address
            for i in grouped
        ])]
